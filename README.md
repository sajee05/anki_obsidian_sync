...
-   **Dependencies:** Just a reminder, it needs `PyYAML` and `markdownify` to work fully.(already present in "1162061440/vendor" folder)
...
* **🚫 Multi-Select Deck Exclusion UI:** Don't want to sync your 10,000-card mature language deck? You can now easily check/uncheck specific decks and sub-decks to completely exclude them from the sync via `Tools > Obsidian Sync > Configure`.
* **🧠 Smart Note-Type Processors:** The sync engine now automatically detects and formats complex community note types to look beautiful in Obsidian:
  * **Image Occlusion Enhanced:** Intelligently separates the Base Image and stacks SVG Question/Answer masks.
  * **Enhanced Cloze V2++:** Parses the main text and automatically blockquotes the `Extra` section.
* **📊 Flawless HTML Tables (Powered by BeautifulSoup):** Say goodbye to shredded markdown tables! The addon now protects complex HTML grids, preserves background colors/borders, and safely un-nests "tables within tables" so your science and geography cards look *exactly* like they do in Anki.
* **🎵 Native Media Embeds:** `[sound:...]` and `<video>` tags are now automatically converted into native Obsidian `![[Audio]]` and `![[Video]]` embeds so you can play them right inside your notes. SVG and WebP images are also now fully supported.
* **🔗 Direct Card Links (CID):** The footer of every synced Obsidian note now includes a clickable deep link using the exact Card ID (`cid:`). Clicking *Anki Reference: [Card 12345]* in Obsidian will instantly launch that exact card in your Anki app!
